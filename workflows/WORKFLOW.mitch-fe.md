...
polling:
  interval_ms: 15000
workspace:
  root: /Users/dm3n/finsider-platform/symphony-workspaces/mitch-fe
hooks:
  after_create: |
    git clone --branch development https://github.com/finsider-ai/Mitch-fe.git .
...
## Repository context: Finsider Mitch-fe

- This is Finsider's Next.js frontend (`finsider-ai/Mitch-fe`).
- The base branch is `development`, NOT `main`. Wherever any skill or instruction
  references `origin/main`, substitute `origin/development`. All PRs target
  `development`.
- Validation: `npm run build` must pass. Run targeted tests when they exist for
  the area you changed.
- UI work MUST use shadcn/ui and the existing local primitives. Preserve
  Finsider's design system: never change global fonts, design tokens, or shared
  chrome unless the ticket explicitly requests it. Keep visual diffs scoped to
  the ticket.
- This project uses `proxy.ts`, not `middleware.ts`. Never create `middleware.ts`.
